...
const generateProvinceLookupTable = ({
  MPs,
  ...args
}: ReturnType<typeof generatePartyLookupTable>): ReturnType<
  typeof generatePartyLookupTable
> & { provinces: ProvinceRecord[] } => ({
  ...args,
  MPs,
  provinces: Object.entries(
    Object.groupBy(MPs, ({ province }) => province),
  ).map(([provinceName, reps]) => ({
    provinceName,
    representativeCount: reps?.length ?? 0,
    partyDistribution: Object.entries(
      Object.groupBy(
        (reps ?? []) as RepresentativeRecord[],
        ({ party }) => party,
      ),
    ).reduce(
      (accum, [party, innerReps]) => ({
        ...accum,
        [party]: innerReps?.length ?? 0,
      }),
      {},
    ),
  })),
...
const resolveAlliances = (term: number, party: string) =>
  Object.entries(
    (
      {
        "27": {
